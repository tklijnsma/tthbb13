...
def Single_Bin_Fits():
    ...
    #for i_part, dic in enumerate(dicts):
    for particle in particles:

        #particle = particles[i_part]
        fit_dicts[particle] = []
        ...
        for i_eta in range( dic['n_eta_bins'] ):
            ...
            for i_E in range( dic['n_E_bins'] ):
                ...
                # Drawing: only do if the ABF results are not going to be added later
                if not config['Draw_ABF_in_single_bins']:

                    # Drawing: Once to pdf, once to png (for convenient HTML
                    # overviews)
                    #   It may be more convenient to make separate loops for fitting
                    #   and drawing.
                    dic['hist_mat'][i_eta][i_E].Draw()

                    # to pdf
                    c1.Print('{3}/{2}/{2}-{0}-{1}'.format(
                        i_eta, i_E, dic['type'], outputdir ), 'pdf')
                    ...
                    img.WriteImage('{3}/{2}png/{2}-{0}-{1}.png'.format(
                        i_eta, i_E, dic['type'], outputdir ) )

            fit_dicts[particle].append( fit_dicts_this_eta )


    # Export the dictionary of fit data to pickle file
    picklef = open( '{0}/pickleSBFfitdata.dat'.format(outputdir), 'wb' )
    pickle.dump( fit_dicts , picklef )
    picklef.close()
    config['SBF_pickle_filename'] = '{0}/pickleSBFfitdata.dat'.format(outputdir)
    ...
    for p in particles:
        ...
        for i_E in range( n_E_bins ):
            for i_eta in range( n_eta_bins ):
                filename = '{0}png/{0}-{1}-{2}.png'.format(
                    p, i_eta, i_E )
                hf.write('<a href="{0}"><img width="350" src="{0}"></a>\n'.format(filename))
            hf.write('<br>\n')
        hf.write( '<br>\n<hr />\n<br>\n</body></html>' )
    ...
    ########################################
    # Writing histogram dictionaries to pickle file
    #   (Needed to plot the across bin fit function results)
    ########################################   

    if config['Draw_ABF_in_single_bins']:
        print 'Writing single bin histograms to pickle file SingleBinHistograms.dat'
        pickle_f = open( '{0}/SingleBinHistograms.dat'.format(outputdir), 'wb' )
